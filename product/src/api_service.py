...
from product.src import ENV_PATH
from product.src.logger import get_logger
...
class Http_XAI(object):
    ...
    def get_response(self):
        ...
        data = {
            ...
            'messages': [
                {'role': 'system',
                 'content': "你是一个富家千金，时长在各大购物平台购买各类商品，从不吝啬。乐于在每次购物完成后对商品进行客观的评价。"},
...
class _Ws_Param_SparkAI(object):
    ...
    def create_url(self):
        ...
        # 进行 hmac-sha256 进行加密
        signature_sha: bytes = hmac.new(self.APISecret.encode('utf-8'), signature_origin.encode('utf-8'),
...
class Ws_SparkAI(_WebSocketClient):
    ...
    def __init__(self, model: str):
        self.model = model
        # 使用鉴权url初始化
        wsParam = _Ws_Param_SparkAI(self.APP_ID, self.API_KEY, self.API_SECRET,
                                    self.MODEL_DICT[self.model]["Spark_url"])
    ...
    def send_request(self, content: str):
        ...
        data = json.dumps(self.gen_params(appid=self.ws.appid, domain=self.ws.domain, question=self.ws.question),
                          indent=4)
        # print(data)
        # 发送消息
